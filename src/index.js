...
const buildPaymailRouter = (baseUrl, config) => {
  ...
  buildVerifyPubkeyRouter(config, (router) => {
    apiRouter.use(router);
    capabilities[CapabilityCodes.verifyPublicKeyOwner] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
      "/verifypubkey/{alias}@{domain.tld}/{pubkey}"
    );
  });
  ...
  buildPublicProfileRouter(config, (router) => {
    ...
    apiRouter.use(router);
    capabilities[CapabilityCodes.publicProfile] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
  ...
  buildReceiveTransactionRouter(config, (router) => {
    // console.warn('This feature is in alpha state. Paymail profile protocol is still being discussed.')
    apiRouter.use(router);
    capabilities[CapabilityCodes.receiveTransaction] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
      "/receive-transaction/{alias}@{domain.tld}"
    );
  });

  buildP2pPaymentDestinationRouter(config, (router) => {
    apiRouter.use(router);
    capabilities[CapabilityCodes.p2pPaymentDestination] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
      "/p2p-payment-destination/{alias}@{domain.tld}"
    );
  });

  buildP2pPaymentDestinationTokenRouter(config, (router) => {
    apiRouter.use(router);
    capabilities[CapabilityCodes.p2pPaymentDestinationWithTokensSupport] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
      "/p2p-payment-destination-tokens-support/{alias}@{domain.tld}"
    );
  });

  buildAssetInformationRouter(config, (router) => {
    apiRouter.use(router);
    capabilities[CapabilityCodes.assetInformation] = joinUrls(
      baseUrl.href,
      getBaseRoute(config),
      "/asset/{alias}@{domain.tld}"
    );
  });
